...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.message.includes('user-not-found') ? 'No account found' : 'Invalid credentials')
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    ...
    >
      ...
      <p className="font-body text-sm text-espresso-500 dark:text-cream-400 mb-8">
        Sign in to your Brewhaus account
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="relative">
          ...
          <button
            ...
            className="absolute right-3.5 top-1/2 -translate-y-1/2 text-espresso-400 dark:text-cream-500 hover:text-espresso-600"
          >
      ...
      <p className="font-body text-sm text-espresso-500 dark:text-cream-400 text-center mt-6">
        ...
        <Link href="/auth/signup" className="text-cream-600 hover:text-cream-500 font-medium transition-colors">
          Sign up
        </Link>
      </p>
    </motion.div>
